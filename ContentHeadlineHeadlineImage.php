...
class ContentHeadlineHeadlineImage extends Frontend
{
	/**
	...
	public function __construct($objTemplate, $isCE = false)
	{
		...
		$objImage = $this->Database->prepare("SELECT addImage, singleSRC, alt, size, imagemargin, fullsize, caption, floating FROM tl_content WHERE id=?")
 						->limit(1)
 						->execute($isCE ? $objTemplate->article : $objTemplate->id);
 		$arrImage = $objImage->fetchAssoc();
 		
 		// add image to template if necessary
 		if($arrImage['addImage'] && $arrImage['singleSRC'] != '' && is_file(TL_ROOT . '/' . $arrImage['singleSRC']) )
 		{
